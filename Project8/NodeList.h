...
class Node {
  ...
  friend class NodeList; // allows direct accessing of link and data from class
                         // NodeList
...
public:
  ...
  DataType &data() { // gets non-const reference, can be used to modify value of
                     // underlying data
    // return const_cast<DataType &>(static_cast<const Node
    // &>(*this).getData());  //an alternative implementation, just for studying
    // reference
    return m_data;
  }
  const DataType &data() const { // gets const reference, can be used to access
                                 // value of underlying data
    return m_data;
  }
